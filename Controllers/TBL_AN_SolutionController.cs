...
namespace NM.APP.CORE.WebAPI.Controllers
{
    ...
    public class TBL_AN_SolutionController : Controller
    {
        private readonly TBL_AN_SolutionService _tBL_AN_SolutionService;
        public TBL_AN_SolutionController(TBL_AN_SolutionService tBL_AN_SolutionService)
        {
            _tBL_AN_SolutionService = tBL_AN_SolutionService;
        }
        ...
        [HttpGet]
        public async Task<IEnumerable<TBL_AN_Solution>> Get()
        {
            return await _tBL_AN_SolutionService.GetAllAsync();
        ...
        {
            var sensor = await _tBL_AN_SolutionService.GetByIdAsync(id);
            if (sensor == null)
            {
                return NotFound();
            }
        ...
        /// </summary>
        /// <param name="tBL_AN_Solution">The sensor entity to create. Must not be null.</param>
        ...
        [HttpPost]
        public async Task<IActionResult> CreateSensor(TBL_AN_Solution tBL_AN_Solution)
        {
            await _tBL_AN_SolutionService.AddAsync(tBL_AN_Solution);
            //var createdSensor = await _sensorService.CreateSensorAsync(sensor);
            return CreatedAtAction(nameof(GetById), new { id = tBL_AN_Solution.Solution_ID }, tBL_AN_Solution);
        }
        ...
        /// object.</param>
        /// <param name="tBL_AN_Solution">An object containing the updated sensor data. The MainTree_ID property must match the value of the id
        /// parameter.</param>
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateSensor(int id, TBL_AN_Solution tBL_AN_Solution)
        {
            if (id != tBL_AN_Solution.Solution_ID)
            {
                return BadRequest();
            }
            await _tBL_AN_SolutionService.UpdateAsync(tBL_AN_Solution);
            return CreatedAtAction(nameof(GetById), new { id = tBL_AN_Solution.Solution_ID }, tBL_AN_Solution);
        ...
        public async Task<IActionResult> DeleteSensor(int id)
        {
            ...
            var deleted = await _tBL_AN_SolutionService.DeleteAsync(id);
            if (deleted == null)
            {
                return NotFound();
            }
